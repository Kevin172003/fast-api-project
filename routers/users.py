...
@router.put("/change_pwd/{user_id}", status_code=status.HTTP_200_OK)
async def change_password(user: user_dependency,user_id: int, db: db_dependency, user_verification: UserVerification):
    if user is None:
        return HTTPException(status_code=401,
                        detail="Could not validate user.")
